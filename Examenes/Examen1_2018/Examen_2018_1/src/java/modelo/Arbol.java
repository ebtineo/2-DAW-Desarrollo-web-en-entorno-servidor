...
public class Arbol implements Serializable {
    ...
    public int coloresConsecutivos() {
        int consecutivos = 0;
        for (int fila = 1; fila < bolas.length; fila ++) {
            for (int columna = 0; columna < fila; columna++) {
                if (bolas[fila][columna].getColor() == bolas[fila][columna+1].getColor()) {
                    consecutivos++;
                }
            }
        }
    ...
    public void cambiar(int fila, int columna) {
        // Obtenemos un número aleatorio entre 0 y 1 para intercambiar la bola
        // por la que está debajo a su derecha o a su izquierda
        int aleatorio = (int)(Math.random() * 2);
        if (fila < bolas.length -1 && columna <= fila) {
            Bola auxiliar = bolas[fila][columna];
            bolas[fila][columna] = bolas[fila+1][columna + aleatorio];
            bolas[fila+1][columna + aleatorio] = auxiliar;
            intentos++;
        }
    }
